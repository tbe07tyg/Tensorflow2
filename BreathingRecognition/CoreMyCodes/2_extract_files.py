...
def extract_files(dataset_Root):
    ...
    for folder in folders:
        video_filenames = glob.glob(os.path.join(dataset_Root, folder, '*.mp4'))

        print("video_filenames len:", len(video_filenames))
...
#
    ...
    # for python 3
    if not os.path.exists('my_data_file.csv'):
        with open('my_data_file.csv', 'w', newline="") as fout:
            writer = csv.writer(fout)
            writer.writerows(data_file)
